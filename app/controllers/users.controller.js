...
const getUsersData = (req, res, next) => {
  ...
  const dataTypes = [
    "username", "name", "email", "description", "verified",
...
const postUser = (req, res, next) => {
  ...
  if (!user.hasOwnProperty("password") || !user.password) return res.status(400).send({ err_msg: "No password provided!" })
  if (!user.hasOwnProperty("verified") || !user.verified) user.verified = false   
  ...
  if (!user.hasOwnProperty("articles")) user.articles = []
  if (!user.hasOwnProperty("comments")) user.comments = []
  if (!user.hasOwnProperty("subscribed_topics")) user.subscribed_topics = []
  if (!user.hasOwnProperty("subscribed_games")) user.subscribed_games = []
  if (!user.hasOwnProperty("followers")) user.followers = []
  if (!user.hasOwnProperty("following")) user.following = []
  if (!user.hasOwnProperty("voted_articles")) user.voted_articles = []
  if (!user.hasOwnProperty("voted_comments")) user.voted_comments = []
...
const patchUserData = (req, res, next) => {
  ...
  const dataTypes = [
    "name", "email", "description", "password", "verified",
